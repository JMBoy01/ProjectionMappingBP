...
def detectAndDrawCirclesPatternFind(img, blobDetector, isImgPatternImg):
    patternSize = (4, 9) # 4 horizontaal, 9 vertical mag 1 schuin geteld worden
...
def drawDetectionsAndSave(imgList, patternImgList, charucoDetector, blobDetector, arucoDetector, path, fileStructureName):
    ...
    while True:
        ...
        if key == 13 and circleCentersProjPlane is not None and circleCentersCam is not None and patternCenters is not None:
            ...
            cv.imwrite("./Validation Dataset/Asym Detected/" + fileStructureName + str(patternImgIndex + 1) + ".png", patternImg)
...
def loadCameraCalibration():
    cameraMatrix =   np.array([[1000, 0, 960], 
                                [0, 1000, 540], 
                                [0, 0, 1]])
...
def main():
    ...
    imgList = loadImageListFromFolder("./Validation Dataset/0_0/", "", ".png")
    patternImgList = loadImageListFromFolder("./Validation Dataset/Asym/", "", ".png")

    print("Detecting charuco board and circle pattern on images...")
    allCircleCentersProjPlane, allCircleCentersCam, allPatternCenters = drawDetectionsAndSave(imgList, patternImgList, charucoDetector, blobDetector, arucoDetector, "./Validation Dataset/0_0 Detected", "detected")
    ...
    cameraMatrix, distCoeffs = loadCameraCalibration()
